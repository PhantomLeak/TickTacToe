...
fn draw_board(state: &[char]) {
    println!("\n");

    // A reverse for loop between 0 -> 3
    for i in (0..3).rev() {
        let offset = i * 3;
...
fn ask_for_user_input(state: &mut [char], player: char) {
    loop {
        print!("Player '");
        print_player(&player);
        ...
        if let Ok(number) = input.trim().parse::<usize>() {
            if number < 1 || number > 9 {
                println!("The field number must be between 1 & 9");
                continue;
            }
...
fn main() {
    ...
    loop {
        ...
        // Ask for user input
        ask_for_user_input(&mut state, player);
        ...
        if is_over(&state) {
            draw_board(&state);
            println!("All fields are used...No winner");
            break;
        }
